...
class CanAd {
    ...
    static let shared = CanAd()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func postulationLikeType(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func value(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
